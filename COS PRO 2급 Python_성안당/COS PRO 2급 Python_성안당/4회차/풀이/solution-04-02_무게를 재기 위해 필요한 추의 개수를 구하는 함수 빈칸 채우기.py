...
def solution(arr, payload):
    answer = 0
    sum = func_a(arr)# 추의 총합이 구슬 무게보다 작으면 잴 수 없다.
    if sum >= payload:
        arr.sort()# 크기 순으로 정렬
        arr.reverse()# 단위가 큰 추를 왼쪽(0번)으로 둠
        weight = 0
        for i in range(len(arr)):
            diff = payload - weight     # 구슬의 무게와 추의 합의 차이
            if arr[i] <= diff:          # 무게 차이보다 크면 구슬의 무게보다 크게된다.
                weight += arr[i]
                print(arr[i], end=' ')
                answer += 1
        print('')
        if weight != payload:# 무게 추 합이 구슬 무게와 다를 때
            answer = 0
    return answer
